...
- [ ] Extend virtual memory beyond the first slice.
  TLBs, selective `sfence.vma`, hardware-managed A/D bits, the first `SUM`/`MXR` supervisor permission controls, `satp`-scoped TLB namespaces, basic global (`PTE.G`) mappings, end-to-end superpage coverage, a user-facing translation-caching microbenchmark for cold/warm loads, ASID switches, global mappings, and `sfence.vma` reloads, plus versioned program-suite coverage for ASID namespace switching, pagetable remap visibility, and superpage data access are now in place. Broader ASID experiments and wider page-table studies remain future work.
