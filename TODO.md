...
- [ ] Extend virtual memory beyond the first slice.
  TLBs, `sfence.vma`, and the first `SUM`/`MXR` supervisor permission controls are now in place. Finer-grained `sfence.vma` behavior, A/D-bit management, ASID-aware experiments, and broader page-table studies remain future work.
