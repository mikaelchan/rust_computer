...
- [ ] Extend virtual memory beyond the first slice.
  TLBs, selective `sfence.vma`, hardware-managed A/D bits, the first `SUM`/`MXR` supervisor permission controls, `satp`-scoped TLB namespaces, basic global (`PTE.G`) mappings, end-to-end superpage coverage, reserved non-leaf `A/D/U`-bit shape checks, malformed superpage leaf-shape checks, a user-facing translation-caching microbenchmark for cold/warm loads, ASID switches, global mappings, and `sfence.vma` reloads, plus versioned program-suite coverage for ASID namespace switching, combined VA+ASID selective fence invalidation, pagetable remap visibility, malformed non-leaf page-table shapes, malformed superpage leaves, superpage data access, and global-mapping survival across ASID-specific `sfence.vma` are now in place. Broader ASID experiments and wider page-table studies remain future work.
...
- [ ] Defer the remaining architectural test backlog into smaller follow-on batches.
  The current program suite already covers a large shared regression surface, so the remaining work can wait while implementation focus moves back to non-test surfaces. Likely future additions are higher-half counter visibility and trap corners beyond the current low-half supervisor/user `cycle`/`time`/`instret` positive and trap slices plus user `cycleh`/`timeh` positive coverage, more malformed-page and permission-shape regressions around superpages plus ASID-targeted fences, and narrower privilege/trap precision corners that are still only covered by unit-style core tests.
