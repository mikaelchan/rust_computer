...
- [ ] Extend privilege support beyond the first supervisor slice.
  Supervisor trap/interrupt state, `satp`-driven translation, machine-mode `MPRV` data accesses with `SUM`/`MXR` coverage, supervisor-routable external interrupt delivery from the interrupt controller and DMA completion path, and supervisor gating through `mstatus.TVM`/`mstatus.TSR` are now in place. Additional supervisor-capable devices, fuller privileged CSR semantics, and nested interrupt behavior remain future work.
