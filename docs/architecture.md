...
- `rvsim-devices`: RAM, ROM, a minimal UART device, and a machine timer MMIO device.
- `rvsim-computer`: a tiny executable that wires the pieces together.

The current milestone targets `RV32I + machine mode CSRs + unified memory + external timer interrupts`. The directory layout already leaves space for branch prediction, richer traps, more devices, and a later out-of-order core.
